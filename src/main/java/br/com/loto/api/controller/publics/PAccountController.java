...
public class PAccountController {
    ...
    public ResponseEntity<CustomResponse<Account>> create(@RequestBody @Valid CreateAccountRequest request) {
        return new ResponseEntity<>(accountPublicService.create(request), HttpStatus.OK);
    }

    @Operation(
            summary = "Solicitar alteração da senha.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "OK",
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = CustomResponse.class))),
    ...
    public ResponseEntity<CustomResponse> resetPassword(@RequestBody @Valid ResetPasswordPublicRequest request) {
        accountPublicService.resetPassword(request);
    ...
    @Operation(
            summary = "Alterar senha utilizando o token.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "OK",
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = CustomResponse.class))),
    ...
    public ResponseEntity<CustomResponse> changePassword(@RequestBody @Valid ChangePasswordPublicRequest request) {
        accountPublicService.changePassword(request);
        return new ResponseEntity<>(CustomResponse.builder()
                .status(200)
